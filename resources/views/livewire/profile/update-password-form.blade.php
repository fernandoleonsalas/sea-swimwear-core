...
new class extends Component
{
    ...
    public int $key = 0; // sirve para forzar el reenderizado
    ...
    public function updatePassword(): void
    {
        try {
            ...
            $this->key++;
            session()->flash('message_exitoso', 'La contraseña se ha cambiado exitosamente.');
...
<section>
    ...
    @if (session()->has('message_exitoso'))
        <p class="mt-1  text-green-700">{{ session('message_exitoso') }}</p>
    @else 
         <header>
        ...
        <form wire:submit="updatePassword" class="mt-6 space-y-6">
            ...
            <div class="flex items-center gap-4">
                <x-primary-button class="bg-green-700 hover:bg-green-400/50">{{ __('Save') }}</x-primary-button>
